...
if( !class_exists('QcSBDPluginUpgradeToProNotice') )
{
	class QcSBDPluginUpgradeToProNotice {
		...
		function func_show_upgrade_link_with_action_links( $links ){
			...
			$links = array_merge( $links, array(
				'<a title="Settings" class="'.$this->link_class.'" style="" href="' . esc_url( admin_url('edit.php?post_type=pnd&page=sbd_settings') ) . '" target="">' . __( 'Settings', 'quantumcloud' ) . '</a>'
			) );

			$links = array_merge( $links, array(
				'<a title="Help" class="'.$this->link_class.'" style="" href="' . esc_url( admin_url('edit.php?post_type=pnd&page=sbd_settings#help') ) . '" target="">' . __( 'Help', 'quantumcloud' ) . '</a>'
			) );
		...
		function func_qc_external_upgrade_link()
		{
		    ...
		    $link_text = '<span class="qc-up-pro-link" style="font-weight: bold; padding: 5px; background: #FCB214; border-radius: 4px; color: #0000ff">'.esc_html($this->link_text).'</span>';
	...
	} //End of class QcSBDPluginUpgradeToProNotice
...
$instance_sbdf2 = new QcSBDPluginUpgradeToProNotice();
