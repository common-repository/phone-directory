...
if ( ! defined( 'qc_sbd_support_path' ) ) {
    define('qc_sbd_support_path', plugin_dir_path(__FILE__));
}

if ( ! defined( 'qc_sbd_support_url' ) )
    define('qc_sbd_support_url', plugin_dir_url( __FILE__ ) );

if ( ! defined( 'qc_sbd_img_url' ) )
    define('qc_sbd_img_url', qc_sbd_support_url . "/images" );


/*Callback function to add the menu */
function qc_sbd_show_promo_page_callback_func(){
    ...
    add_submenu_page(
        ...
        'manage_options',
        "qcopd_sbd_supports",
        'qc_sbd_promo_support_page_callback_func'
    );
    
} //show_promo_page_callback_func

add_action( 'admin_menu', 'qc_sbd_show_promo_page_callback_func', 10 );
...
if ( ! function_exists( 'qc_sbd_include_promo_page_scripts' ) ) {	
	function qc_sbd_include_promo_page_scripts( ) {   


        if( isset($_GET["page"]) && !empty($_GET["page"]) && (   $_GET["page"] == "qcopd_sbd_supports"  ) ){

            wp_enqueue_style( 'qcld-support-fontawesome-css', qc_sbd_support_url . "css/font-awesome.min.css");                              
            wp_enqueue_style( 'qcld-support-style-css', qc_sbd_support_url . "css/style.css");

            wp_enqueue_script( 'jquery' );
            wp_enqueue_script( 'jquery-ui-core');
            wp_enqueue_script( 'jquery-ui-tabs' );
            wp_enqueue_script( 'jquery-sbd-form-processor', qc_sbd_support_url . 'js/support-form-script.js',  array('jquery', 'jquery-ui-core','jquery-ui-tabs') );

            wp_add_inline_script( 'jquery-sbd-form-processor', 
                                    'var qc_sbd_ajaxurl    = "' . admin_url('admin-ajax.php') . '";
                                    var qc_sbd_ajax_nonce  = "'. wp_create_nonce( 'qc-clr' ).'";   
                                ', 'before');
            
        }
	   
	}
	add_action('admin_enqueue_scripts', 'qc_sbd_include_promo_page_scripts');
...
include_once qc_sbd_support_path . '/qc-clr-recommendbot-support-plugin.php';

if ( ! function_exists( 'qc_sbd_promo_support_page_callback_func' ) ) {

	function qc_sbd_promo_support_page_callback_func() {
		
?>


        <div class="qc-sbd-support qcld-support-new-page">
            ...
            <div class="qcld-plugins-lists">
                <div class="qcld-plugins-loading">
                    <img src="<?php echo esc_url(qc_sbd_img_url); ?>/loading.gif" alt="loading">
                </div>
            </div>
        </div>
 ...
 *******************************/
add_action( 'wp_ajax_qc_sbd_process_qc_promo_form', 'qc_sbd_process_qc_promo_form' );

if( !function_exists('qc_sbd_process_qc_promo_form') ){

    function qc_sbd_process_qc_promo_form(){
